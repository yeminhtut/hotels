...
function _cities() {
	...
	$tw=explode('/', $_SERVER['REQUEST_URI']);	
  	$tw[2]= ucwords(str_replace('-', ' ', $tw[3]));
	$check_destination=new Destination();
  	$dest = $check_destination->retrieve_one("name=?", $tw[3]);
	...
	$content['TextHeaderOne'] = $CityName.", ".$CountryName." Tour Package Deals from Singapore";
